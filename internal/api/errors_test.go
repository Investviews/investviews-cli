...
func TestUnknownPlaceAndNotCoveredAreDifferentAnswers(t *testing.T) {
	rec := newClient(t, func(w http.ResponseWriter, r *http.Request) {
		freeHeaders(w)
		writeJSON(w, 404, `{"error":"unknown_place","message":"Nothing matched.","docs_url":"d","query":"zzzznotaplace","did_you_mean":["Zaragoza","Zamora"]}`)
	})
	...
	if len(unknown.DidYouMean) != 2 || unknown.DidYouMean[0] != "Zaragoza" {
